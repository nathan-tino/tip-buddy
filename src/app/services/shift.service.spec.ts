...
describe('ShiftService', () => {
	...
	describe('triggerRefresh', () => {
		it('should trigger refresh and increment refresh signal', () => {
			// Get initial value
			const initialValue = service.refreshTrigger$();

			// Trigger refresh
			service.triggerRefresh();

			// Verify the signal was incremented
			expect(service.refreshTrigger$()).toBe(initialValue + 1);
		});
		...
		it('should increment refresh signal multiple times', () => {
			...
			// Verify the signal was incremented by 3
			expect(service.refreshTrigger$()).toBe(initialValue + 3);
		});

		it('should have readonly refresh trigger signal', () => {
			// Verify the signal is readonly (this test ensures the public API is correct)
			expect(service.refreshTrigger$).toBeDefined();
			expect(typeof service.refreshTrigger$).toBe('function');

			// Verify we can read the value
			const value = service.refreshTrigger$();
			expect(typeof value).toBe('number');

			// The signal should be readonly - we can't directly set it
			// (This is ensured by TypeScript, but we test the API exists)
			expect(service.refreshTrigger$).toBeDefined();
		});

		it('should allow components to subscribe to refresh trigger changes', () => {
			let receivedValues: number[] = [];

			// Create an effect to track signal changes
			const initialValue = service.refreshTrigger$();
			receivedValues.push(initialValue);

			// Trigger some refreshes
			service.triggerRefresh();
			receivedValues.push(service.refreshTrigger$());

			service.triggerRefresh();
			receivedValues.push(service.refreshTrigger$());

			// Verify the values are incrementing correctly
